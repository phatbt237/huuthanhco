...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const updated = await updateItem<EquipmentAssetRecord>(STORE_FILE, id, (existing) => ({
    ...existing,
    ...payload,
    updatedAt: new Date().toISOString(),
  }));

  if (!updated) return Response.json({ error: "Không tìm thấy thiết bị." }, { status: 404 });
...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) {
  const auth = await authenticateAdminUser(request);
  if (auth.response) return auth.response;
  if (!canDelete(auth.user.role, "equipment")) return forbidden();

  const { id } = await params;
  const removed = await deleteItem(STORE_FILE, id);
  if (!removed) return Response.json({ error: "Không tìm thấy thiết bị." }, { status: 404 });
